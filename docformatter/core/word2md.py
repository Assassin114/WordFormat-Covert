...
class Word2MDConverter:
    ...
    def convert(self, docx_path: str, output_path: str = None, preserve_images: bool = True) -> str:
        """
        转换Word文档为Markdown
        ...
        Args:
            ...
            preserve_images: 是否保留图片（保存到media目录）
        ...
        for para in doc.paragraphs:
            md_line = self._convert_paragraph(para, doc)
            if md_line:
    ...
    def _convert_paragraph(self, para, doc) -> str:
        """转换单个段落为Markdown"""
        ...
        # 列表项检测
        if self._is_list_item(para, doc):
            prefix = self._get_list_prefix(para, doc)
            return prefix + md_text
        
        # 引用块
        if align == WD_ALIGN_PARAGRAPH.CENTER and text.startswith('"') and text.endswith('"'):
            return '> ' + md_text
    ...
    def _is_list_item(self, para, doc) -> bool:
        """检测是否是列表项"""
        # 检查paragraph的numPr属性
    ...
    def _get_list_prefix(self, para, doc) -> str:
        """获取列表前缀（- 或 1.）"""
        # 简化处理：返回无序列表
        # 实际应该检查numPr来确定是有序还是无序
        return '- '
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("用法: python3 word2md.py <input.docx> [output.md]")
        sys.exit(1)
